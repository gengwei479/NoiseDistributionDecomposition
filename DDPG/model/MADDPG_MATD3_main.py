...
class Runner:
    def __init__(self, args, env_name, number, seed):
        ...
        self.seed = seed
        # Create env
        if env_name == "simple_speaker_listener":
            self.env = simple_speaker_listener_v3.parallel_env(continuous_actions = True)
            self.env_evaluate = simple_speaker_listener_v3.parallel_env(continuous_actions = True)
            self.env.reset()
        elif env_name == "simple_spread":
            self.env = simple_spread_v2.parallel_env(continuous_actions = True)
            self.env_evaluate = simple_spread_v2.parallel_env(continuous_actions = True)
            self.env.reset()
            
        # self.env = make_env(env_name, discrete=False)  # Continuous action space
        # self.env_evaluate = make_env(env_name, discrete=False)
        self.args.N = len(self.env.agents)  # The number of agents
        self.args.obs_dim_n = [self.env.observation_space(id).shape[0] for id in self.env.agents]  # obs dimensions of N agents
        self.args.action_dim_n = [self.env.action_space(id).shape[0] for id in self.env.agents]  # actions dimensions of N agents
    ...
    def run(self, ):
        ...
        while self.total_steps < self.args.max_train_steps:
            ...
            for _ in range(self.args.episode_limit):
                ...
                # a_n = [agent.choose_action(obs, noise_std=self.noise_std) for agent, obs in zip(self.agent_n, list(obs_n.values()))]
                a_n = {}
                for id, agent in enumerate(self.agent_n):
                    a_n[self.env.agents[id]] = agent.choose_action(obs_n[self.env.agents[id]], noise_std=0)
                # print(dict(zip(self.env.agents, a_n)))
                # print(type(a_n[self.env.agents[0]]))
                # --------------------------!!!注意！！！这里一定要deepcopy，MPE环境会把a_n乘5-------------------------------------------
                # a_n = {'speaker_0': np.array([0.0001, 0.0001, 0.0001]), 'listener_0': np.array([0.06829886, 0.00121086, 0.01114383, 0.0001    , 0.00299623])}
                # a_n = {'speaker_0': self.env.action_space(self.env.agents[0]).sample(), 'listener_0': self.env.action_space(self.env.agents[1]).sample()}
                obs_next_n, r_n, done_n, _, _ = self.env.step(a_n)
                # obs_next_n, r_n, done_n, _ = self.env.step(copy.deepcopy(a_n))
                # Store the transition
                self.replay_buffer.store_transition(list(obs_n.values()), list(a_n.values()), list(r_n.values()), list(obs_next_n.values()), list(done_n.values()))
                obs_n = obs_next_n
                self.total_steps += 1
    ...
    def evaluate_policy(self, ):
        ...
        for _ in range(self.args.evaluate_times):
            ...
            episode_reward = 0
            for _ in range(self.args.episode_limit):
                # a_n = [agent.choose_action(obs, noise_std=0) for agent, obs in zip(self.agent_n, list(obs_n.values()))] # We do not add noise when evaluating
                a_n = {}
                for id, agent in enumerate(self.agent_n):
                    a_n[self.env.agents[id]] = agent.choose_action(obs_n[self.env.agents[id]], noise_std=0)
                # a_n = {'speaker_0': self.env.action_space(self.env.agents[0]).sample(), 'listener_0': self.env.action_space(self.env.agents[1]).sample()}
                # print(a_n)
                obs_next_n, r_n, done_n, _, _ = self.env_evaluate.step(a_n)
                # print(r_n)
                # observations, rewards, terminations, truncations, infos
                episode_reward += sum(list(r_n.values()))#r_n[0]
                obs_n = obs_next_n
                if all(done_n):
                    break
