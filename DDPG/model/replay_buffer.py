...
class ReplayBuffer(object):
    def __init__(self, args):
        ...
        self.buffer_size = args.DDPG_v2_buffer_size
        self.batch_size = args.DDPG_v2_batch_size
        self.count = 0
        self.current_size = 0
        self.buffer_obs_n, self.buffer_a_n, self.buffer_r_n, self.buffer_s_next_n, self.buffer_done_n = [], [], [], [], []
        for agent_id in range(self.N):
            self.buffer_obs_n.append(np.empty((self.buffer_size, args.state_dim[agent_id])))
            self.buffer_a_n.append(np.empty((self.buffer_size, args.action_dim[agent_id])))
            self.buffer_r_n.append(np.empty((self.buffer_size, 1)))
            self.buffer_s_next_n.append(np.empty((self.buffer_size, args.state_dim[agent_id])))
