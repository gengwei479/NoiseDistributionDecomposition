...
import time
import numpy as np
from QMIX_v2.model.utils import get_state_dim, get_dim_from_space
from QMIX_v2.model.policy import QMixPolicy
from QMIX_v2.model.agent import Agent
from QMIX_v2.model.buffer import Buffer
from tensorboardX import SummaryWriter
import os

class Qmix(object):

    def __init__(self, name,  env, arglist):
        self.name = name
        # non-tunable hyperparameters are in args
        self.args = arglist
        self.device = torch.device('cuda:0' if torch.cuda.is_available() else 'cpu')

        self.batch_size = self.args.QMIX_v1_batch_size
        self.hard_update_interval_episode = self.args.QMIX_v1_hard_update_interval_episode
        self.train_interval_episode = self.args.QMIX_v1_train_interval_episode
        self.train_interval = self.args.QMIX_v1_train_interval
        self.test_interval = self.args.QMIX_v1_test_interval
        self.save_interval = self.args.QMIX_v1_save_interval
        self.log_interval = self.args.QMIX_v1_log_interval
        self.grad_norm_clip = self.args.QMIX_v1_grad_norm_clip
        self.buffer_size = self.args.QMIX_v1_buffer_size
        self.use_soft_update = self.args.QMIX_v1_use_soft_update

        # get info to create policies
        cent_obs_dim, cent_act_dim, _ = get_state_dim(env.observation_space, env.action_space)
        self.args.cent_obs_dim = cent_obs_dim
        # cent_obs_dim, cent_act_dim, _ = get_state_dim(arglist.state_dim, arglist.action_dim)
        if self.args.QMIX_v1_share_policy:
            self.args.policy_info = {
                'policy_0': {"cent_obs_dim": cent_obs_dim,
                            "obs_dim": get_dim_from_space(env.observation_space[0]),
                            "act_space": env.action_space[0]}
            }
            self.args.policy_mapping_fn = lambda id: 'policy_0'
        else:
            self.args.policy_info = {
            ...
            self.args.policy_mapping_fn = lambda id: 'policy_' + str(id)
        
        self.policy_info = self.args.policy_info
        self.policy_ids = sorted(list(self.policy_info.keys()))
        self.policy_mapping_fn = self.args.policy_mapping_fn
        ...
        self.log_dir = str(os.getcwd()) + '/QMIX_v2' + str(arglist.log_dir) + str(arglist.env) + '/' + str(arglist.scene_name) + '/' 
        if not os.path.exists(self.log_dir):
            os.makedirs(self.log_dir)

        self.save_dir = os.getcwd() + '/QMIX_v2' + str(arglist.model_dir) + str(arglist.env) + '/' + str(arglist.scene_name) + '/' 
        if not os.path.exists(self.save_dir):
            os.makedirs(self.save_dir)

        # initialize all the policies and organize the agents corresponding to each policy
        self.policies = {p_id: QMixPolicy(self.args, self.policy_info[p_id]) for p_id in self.policy_ids}
        # self.restore(self.save_dir)
        ...
        self.chunk_len = self.args.QMIX_v1_episode_length

        self.buffer = Buffer(self.buffer_size, self.episode_length,
                                      self.policy_ids,
                                      self.agent_ids,
                                      self.policy_agents,
                                      self.policy_obs_dim,
        ...
        self.trainer = Agent(self.args, self.agent_ids, self.policies, self.policy_mapping_fn, self.logger,
        ...
        num_warmup_episodes = max((self.batch_size, self.args.QMIX_v1_num_random_episodes))
    ...
    def main_run_mpe(self):
        ...
        score_history = []
        t_start = time.time()
        for i in range(self.args.QMIX_v1_num_env_steps):
            score = self.train()
            ...
            if len(score_history) >= 100:
                ...
                print('episode', i, 'avg score %.1f' % avg_score, 'time_steps', i, 'time', round(time.time() - t_start, 3))
    ...
    def collect_rollout(self, explore=True, training_episode=True, warmup=False):
        ...
        # while env.agents:
            ...
            for i in range(self.num_envs):
                ep_rewards[i] += sum(list(rew[i].values()))
                # ep_rewards[i] += list(rew[i].values())[0]  # shared reward so take any reward
                # TODO: change to allow for unshared reward
    ...
    def collect_test_rollouts(self):
        ...
        successes = []
        scores = []
        for _ in range(self.args.QMIX_v1_num_test_episodes):
            if self.use_available_actions:
                if self.take_turn:
        ...
        if self.use_available_actions:
            if self.take_turn:
                ...
                battles_won_rate = np.sum(np.array(battles_wons))/self.args.QMIX_v1_num_test_episodes
